...
@section('title', 'Tambah Jadual - Sistem e-Invigilator UiTM')

@section('content')
<div class="main">
    <div class="main-header">
        <h1 class="main-title">Tambah Jadual Baharu</h1>
        ...
        <form action="{{ route('admin.storeSchedule') }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem;">
                <div>
                    ...
                    <input type="text" id="userID" name="userID" value="{{ old('userID') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="userName" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Nama</label>
                    <input type="text" id="userName" name="userName" value="{{ old('userName') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                ...
                <div>
                    ...
                    <select id="position" name="position" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                        ...
                        @foreach($jawatan as $jabatan)
                            <option value="{{ $jabatan }}" {{ old('position') == $jabatan ? 'selected' : '' }}>{{ $jabatan }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="role" name="role" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                        ...
                        @foreach($tugas as $tugasItem)
                            <option value="{{ $tugasItem }}" {{ old('role') == $tugasItem ? 'selected' : '' }}>{{ $tugasItem }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="examDate" name="examDate" value="{{ old('examDate') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="examDay" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Hari Peperiksaan</label>
                    <input type="text" id="examDay" name="examDay" value="{{ old('examDay') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;" readonly>
                </div>
                
                <div>
                    <label for="startTime" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Masa Mula</label>
                    <input type="time" id="startTime" name="startTime" value="{{ old('startTime') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="endTime" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Masa Tamat</label>
                    <input type="time" id="endTime" name="endTime" value="{{ old('endTime') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="programCode" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Kod Program</label>
                    <input type="text" id="programCode" name="programCode" value="{{ old('programCode') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="courseCode" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Kod Kursus</label>
                    <input type="text" id="courseCode" name="courseCode" value="{{ old('courseCode') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="group" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Kumpulan</label>
                    <input type="text" id="group" name="group" value="{{ old('group') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="totalStudent" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Jumlah Pelajar</label>
                    <input type="number" id="totalStudent" name="totalStudent" value="{{ old('totalStudent') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
                
                <div>
                    <label for="venue" style="display: block; margin-bottom: 0.5rem; font-weight: 500;">Tempat</label>
                    <input type="text" id="venue" name="venue" value="{{ old('venue') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                </div>
            </div>

            <div style="margin-top: 2rem;">
                <button type="submit" class="btn btn-success">Simpan Jadual</button>
